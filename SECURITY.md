...
2. Remote, behind an authenticated proxy. Operators set `MCP_AUTH_TOKEN`
   and either bind to a non-loopback interface (`MCP_BIND`) or expose the
   server via a reverse proxy / Cloudflare Access tunnel.

The server refuses to start when bound to a non-loopback interface without
`MCP_AUTH_TOKEN`. This check is enforced in `buildApp` (`src/cli/serve.ts`)
and is the main safety net against accidentally shipping an open-by-default
server.

### What the server protects

- Authentication: a bearer token gates every `/api` and `/mcp` request.
  The `Authorization: Bearer <token>` value is compared in constant time.
...
The network-edge boundary sits at the reverse proxy, where TLS terminates.
The auth boundary sits at the API, where the bearer token is checked in
constant time. The data plane is local-only: SQLite and the embedder run
inside the container alongside the server.
